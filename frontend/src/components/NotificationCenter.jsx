          ...
          >
            <div
              className={`mt-1 rounded-lg p-1.5 ${
                isBullish ? 'bg-emerald-500/20 text-emerald-300' : 'bg-rose-500/20 text-rose-300'
              }`}
            >
              <AlertCircle className="h-4 w-4" />
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-xs uppercase tracking-wide font-semibold opacity-80">{notif.ticker}</p>
              <p className="text-sm mt-1">Moved {Math.abs(notif.movement).toFixed(1)}%</p>
            </div>
            <button
