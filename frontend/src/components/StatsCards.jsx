import { ArrowDownRight, ArrowUpRight, BadgeDollarSign, BarChart3, BriefcaseBusiness } from 'lucide-react'
...
function StatCard({ icon, label, value, subtext, tone = 'neutral', valueClassName = '' }) {
  const Icon = icon
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-3">
      ...
      <StatCard
        ...
        tone={pnlTone}
        valueClassName={metrics.dailyPnL >= 0 ? 'text-emerald-300' : 'text-rose-300'}
      />
      <StatCard
        icon={BriefcaseBusiness}
        label="Total Exposure"
        value={formatNumber(metrics.totalExposure)}
        subtext={metrics.topMover ? `Top mover: ${metrics.topMover.ticker}` : 'Awaiting live holdings data'}
        tone="neutral"
      />
    </div>
  )
