    ...
    <div className="fixed inset-0 z-50 flex items-stretch justify-end bg-slate-950/70 backdrop-blur-sm">
      ...
      <section className="relative z-10 flex h-full w-full max-w-xl flex-col border-l border-white/10 bg-slate-950/95 shadow-2xl shadow-black/40">
        <div className="flex items-start justify-between gap-4 border-b border-white/10 p-6">
          <div>
            <p className="text-xs uppercase tracking-[0.3em] text-slate-400">Trade desk</p>
            <h2 className="mt-2 text-2xl font-semibold text-white">Add New Trade</h2>
            <p className="mt-2 text-sm text-slate-400">Submit a new position and let the backend persist it to portfolio.json.</p>
          </div>
          <button
            type="button"
            onClick={onClose}
        ...
        <form className="flex-1 space-y-5 overflow-y-auto p-6" onSubmit={handleSubmit}>
          <div className="grid gap-4 sm:grid-cols-2">
            <Field label="Ticker" value={form.ticker} onChange={updateField('ticker')} placeholder="NVDA" />
            <Field label="Quantity" value={form.quantity} onChange={updateField('quantity')} placeholder="10" type="number" step="1" min="0" />
            <Field label="Average Cost" value={form.averageCost} onChange={updateField('averageCost')} placeholder="840.00" type="number" step="0.01" min="0" />
            <Field label="Current Price" value={form.currentPrice} onChange={updateField('currentPrice')} placeholder="845.50" type="number" step="0.01" min="0" />
          </div>

          <div className="rounded-3xl border border-white/10 bg-white/5 p-4">
            <div className="flex items-center justify-between gap-4 text-sm text-slate-400">
              <span>Estimated market value</span>
              <span className="font-semibold text-white">{formatCurrency(previewValue)}</span>
            </div>
          </div>
...
function Field({ label, type = 'text', value, onChange, placeholder, step, min }) {
  return (
